...
func initApp(confServer *conf.Server, registry *conf.Registry, confData *conf.Data, tracerProvider *trace.TracerProvider) (*kratos.App, func(), error) {
	...
	codeHeaderRepo := data.NewCodeHeaderRepo(dataData)
	codeHeaderUseCase := biz.NewCodeHeaderCase(codeHeaderRepo, slogUseCase)
	sginSgin := sgin.NewSgin(defaultFileDir, engine, centralUseCase, userUseCase, codeUseCase, codeHeaderUseCase)
